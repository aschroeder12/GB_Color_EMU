...
void RunCPU(unsigned short *beginDotCnt, unsigned short endDotCnt)
{
	...
	case 0x04:
		...
		*beginDotCnt = *beginDotCnt + (1)*4; /* 1 M Cycle * 4 = Dots */
		break;
	case 0x05:
		PrintLog("DEC B \n");
		PC_REGISTER = PC_REGISTER + 1;
		INSTR_DEC_R(&B_REGISTER);
		*beginDotCnt = *beginDotCnt + (1)*4;
		break;
	case 0x06:
		PrintLog("LD B n \n");
		PC_REGISTER = PC_REGISTER + 1;
		INSTR_LOAD_REGISTER_R_N(&B_REGISTER);
		*beginDotCnt = *beginDotCnt + (2)*4;
		break;
	case 0x0d:
		PrintLog("DEC C \n");
		PC_REGISTER = PC_REGISTER + 1;
